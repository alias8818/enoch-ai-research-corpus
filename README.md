...
The strict audit is intentionally separate from the packaging/provenance lint. Current status: **393 / 393 artifacts pass**. The audit reports **0 empty claim ledgers** and **0 missing public result-file references**. That is explicit public evidence accounting, not a scientific correctness verdict and not something hidden by the packaging/provenance pass count.

Do not cite the packaging/provenance lint without saying what it validates. Nearby strict claim/evidence audit status must stay visible beside packaging counts. Use the strict audit when reviewing whether a particular generated claim can be traced to public result files or to explicit unavailability metadata with a public surrogate.
